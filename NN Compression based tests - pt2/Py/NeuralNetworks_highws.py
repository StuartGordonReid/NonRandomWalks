...
def compression_test(sims, quandl_codes, window_sizes, decimation_levels,
    ...
    for ws in window_sizes:
        ...
        for dc in decimation_levels:
            ...
            loss_df.to_csv("Loss_hws.csv")
            loss_df[[col for col in loss_df.columns if "train" in col or "meta" in col]].to_csv("Loss_Train_hws.csv")
            loss_df[[col for col in loss_df.columns if "tests" in col or "meta" in col]].to_csv("Loss_Tests_hws.csv")
            loss_df[[col for col in loss_df.columns if "valid" in col or "meta" in col]].to_csv("Loss_Valid_hws.csv")

            # Write out the accuracy results at this point in time.
            accs_df = pandas.DataFrame(accuracy_results)
            accs_df.columns = names
            accs_df.to_csv("Accuracy_hws.csv")
            accs_df[[col for col in accs_df.columns if "train" in col or "meta" in col]].to_csv("Accuracy_Train_hws.csv")
            accs_df[[col for col in accs_df.columns if "tests" in col or "meta" in col]].to_csv("Accuracy_Tests_hws.csv")
            accs_df[[col for col in accs_df.columns if "valid" in col or "meta" in col]].to_csv("Accuracy_Valid_hws.csv")


ex_windows = list(range(20, 260, 10))
ex_decimations = list(range(1, 5, 1))
