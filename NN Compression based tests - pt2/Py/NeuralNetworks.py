...
def compression_test(sims, quandl_codes, window_sizes, decimation_levels,
    ...
    for ws in window_sizes:
        ...
        for dc in decimation_levels:
            ...
            loss_df[[col for col in loss_df.columns if "train" in col or "meta" in col]].to_csv("Loss_Train.csv")
            loss_df[[col for col in loss_df.columns if "tests" in col or "meta" in col]].to_csv("Loss_Tests.csv")
            loss_df[[col for col in loss_df.columns if "valid" in col or "meta" in col]].to_csv("Loss_Valid.csv")

            # Write out the accuracy results at this point in time.
            accs_df = pandas.DataFrame(accuracy_results)
            accs_df.columns = names
            accs_df.to_csv("Accuracy.csv")
            accs_df[[col for col in accs_df.columns if "train" in col or "meta" in col]].to_csv("Accuracy_Train.csv")
            accs_df[[col for col in accs_df.columns if "tests" in col or "meta" in col]].to_csv("Accuracy_Tests.csv")
            accs_df[[col for col in accs_df.columns if "valid" in col or "meta" in col]].to_csv("Accuracy_Valid.csv")
