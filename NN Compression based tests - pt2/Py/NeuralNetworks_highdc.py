...
def compression_test(sims, quandl_codes, window_sizes, decimation_levels,
    ...
    for ws in window_sizes:
        ...
        for dc in decimation_levels:
            ...
            loss_df.to_csv("Loss_hdc.csv")
            loss_df[[col for col in loss_df.columns if "train" in col or "meta" in col]].to_csv("Loss_Train_hdc.csv")
            loss_df[[col for col in loss_df.columns if "tests" in col or "meta" in col]].to_csv("Loss_Tests_hdc.csv")
            loss_df[[col for col in loss_df.columns if "valid" in col or "meta" in col]].to_csv("Loss_Valid_hdc.csv")

            # Write out the accuracy results at this point in time.
            accs_df = pandas.DataFrame(accuracy_results)
            accs_df.columns = names
            accs_df.to_csv("Accuracy_hdc.csv")
            accs_df[[col for col in accs_df.columns if "train" in col or "meta" in col]].to_csv("Accuracy_Train_hdc.csv")
            accs_df[[col for col in accs_df.columns if "tests" in col or "meta" in col]].to_csv("Accuracy_Tests_hdc.csv")
            accs_df[[col for col in accs_df.columns if "valid" in col or "meta" in col]].to_csv("Accuracy_Valid_hdc.csv")


ex_windows = list(range(4, 20, 2))
